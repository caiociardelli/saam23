...
unsigned readBlockModelHeader (char *dir, char *prm,
                               unsigned *np_b,
                               unsigned *nt_b,
                               unsigned *nr_b)
{
  ...
  if (sprintf (name, "%s/%s.bin", dir, prm) < 12) return 1;
...
unsigned readBlockModel (char *dir, char *prm,
                         unsigned np_b,
                         unsigned nt_b,
                         unsigned nr_b,
                         ...
                         double Theta[nt], double Phi[np])
{
  ...
  double dt = 1.605702911834783 / (nt - 1);
  double dp = 2.129301687433082 / (np - 1);

  double t = 1.1344640137963142;
